# VPC Module - Creates enterprise VPC with public/private subnets

locals {
  availability_zones = var.availability_zones != null ? var.availability_zones : data.aws_availability_zones.available[0].names
}
...
resource "aws_subnet" "database" {
  count             = var.create_database_subnets ? length(local.availability_zones) : 0
  vpc_id            = aws_vpc.main.id
  cidr_block        = cidrsubnet(var.vpc_cidr, 8, count.index + (length(local.availability_zones) * 2))
  availability_zone = local.availability_zones[count.index]
